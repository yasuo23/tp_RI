...
def precision5(sect,filtred):
    s = 0  # Initialisation du compteur
    top5_ids = [id for id, _ in filtred[:5]]
    print(top5_ids )
    print(sect)# Prendre les 5 premiers IDs filtrés
...
def evaluate_relevance(token_value, words, stemmer_value):
    ...
    t = np.zeros((len(words), 1033), dtype=object)
    ...
    try:
        with open(f'./result_dir/description{token_value}{stemmer_value}.txt', 'r') as f:
            for i, word in enumerate(words):
...
def search_action(query_value, token_value, stemmer_value, index_selection, matching_selection,k,b,eva,q):
    ...
    if matching_selection == "without":
      ...
      with open(f'./result_dir/description{token_value}{stemmer_value}.txt', 'r') as f:
        ...
        w = np.zeros((len(words),1033)) 
        t = np.zeros(( len(words),1033)) 
        b=np.zeros(( len(words),1033))
        fr = np.zeros(( len(words),1033))
        sizeW=  np.zeros((1033)) 
    ...
    # Initialize the Porter Stemmer
        ...
        #  print("Jaccard RSV for each document:", rsv)
         ...
         final_list =  rsv_sorted 
          ...
          k = 1.45  # Hyperparameter for term saturation
          b = 0.7  # Hyperparameter for document length normalization
          N = 1033  # Total number of documents
          rsv = []  # Initialize list to store (doc_id, relevance) tuples
        #   print(fr)
        ...
        if eva=='OUI': 
            ...
            #  print(sect)
               # Liste d'identifiants pertinents
